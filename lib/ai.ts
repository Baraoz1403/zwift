...
export interface WeeklyWorkout {
  ...
  title: string;
  durationMin: number;
  /** Cycling only — e.g. "65-75%" of FTP. Omitted for running and rest days. */
  targetPowerPctFtp?: string;
  /** Running only — effort descriptor or pace range, e.g. "Easy (conversational)"
   *  or "5:30–6:00 min/km". Omitted for cycling and rest days. */
  targetPace?: string;
  description: string;
...
` +
  ...
  "hard intervals are 'Interval Run' type, alternating walk/run for beginners or recovery " +
  "is 'Walk/Run' type. For running sessions use targetPace (not targetPowerPctFtp) " +
  "to describe intensity — e.g. 'Easy (conversational)', 'Tempo (comfortably hard)', " +
  "'Hard (near race pace)', or a pace range like '5:30–6:00 min/km'. " +
  "Never output targetPowerPctFtp for running sessions. " +
...
"Staying flat week over week with no stated reason is a coaching failure, not a conservative choice. " +
  ...
  '"durationMin": number, "targetPowerPctFtp": string (cycling only — e.g. "65-75%", ' +
  'omit for running and rest days), "targetPace": string (running only — ' +
  'e.g. "Easy (conversational)" or "5:30-6:00 min/km", omit for cycling and rest), ' +
