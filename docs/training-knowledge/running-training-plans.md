...
**Structure**: 3 run days + 1 walk day per week. Monday/Wednesday/Friday: optional
rest or walk/run. Tuesday & Saturday: run. Sunday: brisk walk.
...
- Walking days count as active recovery, not wasted days
...
**Structure**: 3 runs + 2 cross-train days per week. Monday + Friday: rest.
Tuesday + Thursday: runs. Wednesday + Saturday: cross-training (cycling, swimming,
walking — easy effort). Sunday: long run (the most important session).
...
**Key principles**:
- Sunday long run: slow! This is not a race. Go 60–90 seconds per km slower
  than you think you should.
- Cross-training = easy cycling on Zwift is perfect here (Z1–Z2 only)
- Strength training (if any): after runs, never before
...
**Target rider**: Can run 3 miles (4.8 km) comfortably. Has done some 5K/10K training.

**Structure**: 4 runs + cross-training. Mon + Fri: rest. Tue + Thu: medium runs.
Sat: cross-train. Sun: long run (the key session, building to 16 km by week 11).
Includes a built-in 5K race at week 6 and a 10K race at week 9 as fitness checks.
Final week 12: taper (volume cut ~50%, keep one short run).
...
**Key principles for half-marathon preparation**:
- Long run pace: conversational easy. Never race the long runs.
- Week 12 taper: cut volume roughly 50%, keep intensity (one short tempo run
  is fine). Common mistake = stopping completely → arrive at race flat-legged.
- Cross-training days (Wed + Sat): 30–60 min easy cycling, swimming, or walking.
  Cycling on Zwift at Z1–Z2 is ideal cross-training — aerobic stimulus without
  running impact.
- Strength training: after runs on Tue/Thu. Core work and hip exercises reduce
  injury risk significantly in half marathon training.

---

## Adapting running plans to the Zwift AI context

When a Zwift rider has running in their goal or history:
...
- **Running is higher injury risk than cycling** — if the rider shows high
  training load (lib/training-load.ts), reduce run volume before cycling volume.
...
- Cross-referenced with periodization.md and recovery-week.md
