...
export default function HeroBanner(){
  const[i,si]=useState(0);const[p,sp]=useState(0);const tr=useRef(null);const b=B[i];
  useEffect(()=>{
    if(tr.current)clearInterval(tr.current);sp(0);
    const s=Date.now();
    tr.current=setInterval(()=>{
      const pct=Math.min(((Date.now()-s)/6000)*100,100);sp(pct);
      if(pct>=100){si(x=>(x+1)%3);clearInterval(tr.current);}
    },50);
  ...
  return(
    <div style={{background:b.bg,borderRadius:16,padding:"40px 36px 32px",marginBottom:40,position:"relative",overflow:"hidden",border:"1px solid "+b.a+"20",boxShadow:"0 0 50px "+b.a+"12",fontFamily:"system-ui,sans-serif"}}>
      <div style={{position:"absolute",inset:0,opacity:.035,backgroundImage:"linear-gradient("+b.a+" 1px,transparent 1px),linear-gradient(90deg,"+b.a+" 1px,transparent 1px)",backgroundSize:"36px 36px"}}/>
      ...
      <div style={{position:"relative",zIndex:1}}>
        ...
        {b.h.map((l,j)=>(<div key={j} style={{fontSize:38,fontWeight:800,lineHeight:1.1,color:j===b.h.length-1?b.a:"white",textShadow:j===b.h.length-1?"0 0 24px "+b.a+"55":"none"}}>{l}</div>))}
        <p style={{fontSize:15,color:"#94a3b8",lineHeight:1.7,margin:"16px 0 28px",maxWidth:480}}>{b.s}</p>
        <div style={{display:"flex",gap:10}}>
          {B.map((_,j)=>(<button key={j} onClick={()=>si(j)} style={{border:"none",cursor:"pointer",padding:0,background:"transparent"}}>
