 ...
 *      dashboard itself uses (lib/headless-sync.ts), so a plan that arrived
 *      via cron behaves identically to one generated by hand.
 *
 * TrainingPeaks is intentionally NOT auto-synced here either, for the same
 * reason the interactive path stopped doing it - see the doc comment on
 * syncPlanToConnectedPlatforms in app/dashboard/weekly-plan.tsx.
 */
